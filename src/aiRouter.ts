...
async function json(url:string,init:RequestInit){const r=await fetch(url,init);const body=await r.text();let data:any;try{data=JSON.parse(body)}catch{data={raw:body}}if(!r.ok)throw err(data?.error?.message||data?.message||data?.raw||`Provider request failed (${r.status})`,r.status);return data}
